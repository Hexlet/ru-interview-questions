...
1. Какие способы коммуникации между микросервисами знаешь: брокеры сообщений (Kafka), REST, RPC? Их особенности, плюсы и минусы.
1. Что такое partitions в Kafka? Как устроены, как влияют на масштабирование и порядок сообщений?
1. Что такое паттерн transactional outbox? Для чего нужен? Какие есть альтернативные способы решения проблемы согласованности между БД и брокером?

### Spring/JPA/Hibernate

1. Как работает `@Transactional` в Spring: динамические прокси, перехват вызова, когда открывается/коммитится/роллбэкается транзакция (рантайм vs компиляция)?
1. Будет ли работать `@Transactional` на `private`-методе? Почему (proxy/self-invocation)?
1. Какие подводные камни у `@Transactional` (границы прокси, checked/unchecked исключения, вложенные вызовы)?
1. В чем разница между EAGER и LAZY загрузкой в JPA/Hibernate? Когда что использовать?
1. Что такое проблема N + 1 запросов и как её избежать (fetch join, EntityGraph, batch size, кэш)?
1. Преимущества и недостатки использования ORM.

### Базы данных и производительность

1. С какими БД работал? Как выбирал под конкретную задачу? Какие критерии учитывал?
1. Что такое partitioning и sharding? В чем отличия и когда что использовать?
1. Будет ли работать индекс при использовании `LIKE`? В каких случаях B-tree индекс помогает (поиск по префиксу)?
...
1. CI/CD: что именно входит в CI, а что в CD? Приведи примеры практик.
1. Как организована работа с Git-ветками на проекте? Какие стратегии применялись (GitFlow, Trunk-Based, GitHub Flow)?

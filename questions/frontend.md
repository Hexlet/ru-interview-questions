...
1. Расскажи про замыкания в JS. Приведи пример замыкания. Есть ли такое в других языках?
1. Расскажи про всплытие (hoisting).
1. Расскажи про область видимости.
1. В чем отличие `var`, `let`, и `const`?
1. Расскажи про ключевое слово `this` в JS?
...
1. Как можно привязать контекст к функции?
1. Что делают `call()`, `bind()`, `apply()`?
1. Расскажи про асинхронность в JS. Каким способом она достигается?
1. Что такое Event Loop? Для чего он нужен? Какую проблему решает? Какие есть фазы?
1. Если в Event Loop ничего не останется после выполнения, то что произойдет?
1. В какой последовательности вызываются микро и макро таски?
1. Если поставить таймер на 10 секунд, то через сколько он сработает? Почему?
1. Что такое Promise в JS? Где и как они используются?
...
1. Чем множество `Set` отличается от массива `Array` в Javascript?
...
1. Зачем нужен `debounce()`? Написать реализацию.
...
1. Из каких компонентов состоит NodeJS?

### JavaScript

1. Всё ли в JS ведет себя как объект?
1. Что такое наследование?
1. Расскажи про прототипы.
1. Что дает нам прототипное наследование? Через какое свойство мы наследуем?
1. Что на конце цепочки прототипов?
1. В чем разница `proto` и `prototype`?
...
1. Что такое `WeakSet`, `WeakMap`?
1. Какие есть способы копирования объекта в JavaScript?
...
1. Как мы можем принудительно вынести HTML элемент на слой?
